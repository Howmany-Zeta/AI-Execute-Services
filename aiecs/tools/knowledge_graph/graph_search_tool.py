...
class GraphSearchInput(BaseModel):
    ...
    query_embedding: Optional[List[float]] = Field(
        None,
        description="Query vector embedding (for vector/hybrid search)"
    )
...
class GraphSearchTool(BaseTool):
    ...
    def __init__(self):
        ...
        # Search strategies (using _strategy suffix to avoid shadowing public methods)
    ...
    async def _execute(self, **kwargs) -> Dict[str, Any]:
        ...
        # use_cache is available in kwargs but not currently used in implementation
        ...
        try:
            if mode == SearchModeEnum.VECTOR:
                ...
                results = await self._graph_search(
                    seed_entity_ids, max_depth, max_results
                )

            elif mode == SearchModeEnum.HYBRID:
                results = await self._hybrid_search(
                    query_embedding, seed_entity_ids, entity_type,
                    max_results, max_depth, vector_weight, graph_weight,
                    expand_results, vector_threshold
                )

            elif mode == SearchModeEnum.PAGERANK:
                results = await self._pagerank_search(
                    seed_entity_ids, max_results
                )
                ...
                results = await self._filtered_search(
                    entity_type, property_filters, max_results
                )
    ...
    async def vector_search(
        ...
        return {"success": True, "mode": "vector", "num_results": len(results), "results": results}
    ...
    async def graph_search(
        ...
        return {"success": True, "mode": "graph", "num_results": len(results), "results": results}
    ...
    async def hybrid_search(
        ...
        if query and not query_embedding:
            query_embedding = [0.1] * 128  # Placeholder
        results = await self._hybrid_search(
            query_embedding, seed_entity_ids, entity_type, max_results, max_depth,
            vector_weight, graph_weight, expand_results, vector_threshold
        )
        return {"success": True, "mode": "hybrid", "num_results": len(results), "results": results}
    ...
    async def pagerank_search(
        ...
        return {"success": True, "mode": "pagerank", "num_results": len(results), "results": results}
    ...
    async def multihop_search(
        ...
        return {"success": True, "mode": "multihop", "num_results": len(results), "results": results}
    ...
    async def filtered_search(
        ...
        return {"success": True, "mode": "filtered", "num_results": len(results), "results": results}
    ...
    async def traverse_search(
        ...
        return {"success": True, "mode": "traverse", "num_results": len(results), "results": results}
