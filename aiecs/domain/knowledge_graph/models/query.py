...
class GraphQuery(BaseModel):
    ...
    entity_id: Optional[str] = Field(
        default=None,
        description="Entity ID for entity lookup queries"
    )
    ...
    relation_type: Optional[str] = Field(
        default=None,
        description="Filter by relation type (for traversal)"
    )

    # Vector search
    embedding: Optional[List[float]] = Field(
        default=None,
        description="Query embedding for vector search"
    )

    # Property constraints
    properties: Dict[str, Any] = Field(
        default_factory=dict,
        description="Property constraints for filtering"
    )
...
class GraphResult(BaseModel):
    ...
    entities: List[Entity] = Field(
        ...
        description="List of entity results"
    )
    ...
    scores: List[float] = Field(
        default_factory=list,
        description="Scores for each entity result"
    )
    ...
    def __str__(self) -> str:
        parts = [
            f"GraphResult(entities={self.entity_count}, paths={self.path_count})"
        ]
