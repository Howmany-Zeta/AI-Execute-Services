...
class SkillScriptExecutor:
    ...
    def __init__(self, default_timeout: int = 30, max_timeout: int = 600, max_output_size: int = 1024 * 1024):  # 1MB
        """
    ...
    async def execute(
        ...
        try:
            ...
            if not str(script_path).startswith(str(skill_root)):
                return ScriptExecutionResult(success=False, result=None, error=f"Script path must be within skill directory: {script_path}", mode_used=mode)
        except Exception as e:
            return ScriptExecutionResult(success=False, result=None, error=f"Invalid path: {e}", mode_used=mode)

        if not script_path.exists():
            return ScriptExecutionResult(success=False, result=None, error=f"Script not found: {script_path}", mode_used=mode)

        # Determine actual mode
        actual_mode = self._resolve_mode(script_path, mode)

        if actual_mode == ExecutionMode.NATIVE:
            return await self._execute_native(script_path, skill_root, input_data, timeout)
        else:
            return await self._execute_subprocess(script_path, skill_root, input_data, timeout, env_vars)

    def _resolve_mode(self, script_path: Path, mode: ExecutionMode) -> ExecutionMode:
        """
    ...
    async def _execute_native(self, script_path: Path, skill_root: Path, input_data: Optional[Dict[str, Any]], timeout: Optional[int]) -> ScriptExecutionResult:
        """
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                return ScriptExecutionResult(success=False, result=None, error=f"Cannot load script: {script_path}", execution_time=time.time() - start_time, mode_used=ExecutionMode.NATIVE)

            module = importlib.util.module_from_spec(spec)

            # Add skill_root to module for resource access
            setattr(module, "__skill_root__", skill_root)
            ...
            if entry_point is None:
                return ScriptExecutionResult(
                    success=False, result=None, error=f"Script missing entry point ({', '.join(self.ENTRY_POINTS)})", execution_time=time.time() - start_time, mode_used=ExecutionMode.NATIVE
                )

            # Execute with timeout
            if asyncio.iscoroutinefunction(entry_point):
                # Async function
                result = await asyncio.wait_for(entry_point(input_data or {}), timeout=effective_timeout)
            else:
                # Sync function - run in executor
                loop = asyncio.get_running_loop()
                result = await asyncio.wait_for(loop.run_in_executor(None, entry_point, input_data or {}), timeout=effective_timeout)

            return ScriptExecutionResult(success=True, result=result, execution_time=time.time() - start_time, mode_used=ExecutionMode.NATIVE)

        except asyncio.TimeoutError:
            return ScriptExecutionResult(
                success=False, result=None, error=f"Script execution timed out after {effective_timeout}s", execution_time=time.time() - start_time, mode_used=ExecutionMode.NATIVE, timed_out=True
            )
        except Exception as e:
            return ScriptExecutionResult(
                success=False, result=None, error=f"Script execution failed: {type(e).__name__}: {str(e)}", execution_time=time.time() - start_time, mode_used=ExecutionMode.NATIVE
            )

    async def _execute_subprocess(self, script_path: Path, skill_root: Path, input_data: Optional[Dict[str, Any]], timeout: Optional[int], env_vars: Optional[Dict[str, str]]) -> ScriptExecutionResult:
        """
        ...
        try:
            process = await asyncio.create_subprocess_exec(*cmd, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, env=env, cwd=str(skill_root))

            try:
                stdout, stderr = await asyncio.wait_for(process.communicate(input=stdin_data), timeout=effective_timeout)
            except asyncio.TimeoutError:
                timed_out = True
                process.kill()
                await process.wait()
            ...
            return ScriptExecutionResult(success=False, result=None, error=f"Subprocess execution failed: {str(e)}", execution_time=time.time() - start_time, mode_used=ExecutionMode.SUBPROCESS)
