...
class SkillLoader:
    ...
    async def load_skill(self, skill_path: Path, load_body: bool = True) -> SkillDefinition:
        """
        ...
        try:
            ...
            return SkillDefinition(metadata=metadata, skill_path=skill_path, body=body if load_body else None, references=references, examples=examples, scripts=scripts, assets=assets)
    ...
    def _parse_frontmatter(self, content: str) -> Tuple[Dict[str, Any], str]:
        ...
        if not match:
            raise SkillLoadError("SKILL.md must have YAML frontmatter delimited by '---'")
    ...
    async def _discover_resources(self, skill_path: Path, directory_name: str, resource_type: str) -> Dict[str, SkillResource]:
        """
        ...
        try:
            for file_path in directory.iterdir():
                if file_path.is_file() and not file_path.name.startswith("."):
                    ...
                    resources[resource_name] = SkillResource(path=relative_path, type=resource_type, content=None, executable=False)  # Lazy loaded
        except OSError as e:
            logger.warning(f"Failed to scan directory {directory}: {e}")

        return resources

    async def _discover_scripts(self, skill_path: Path, scripts_config: Dict[str, Any]) -> Dict[str, SkillResource]:
        """
        ...
        for script_name, config in scripts_config.items():
            if isinstance(config, dict):
                ...
                if not script_path:
                    logger.warning(f"Script '{script_name}' missing 'path' in YAML config")
                    continue

                # Validate script path exists
                full_path = skill_path / script_path
                if not full_path.exists():
                    logger.warning(f"Script path does not exist: {script_path} " f"(declared in YAML for '{script_name}')")
                    continue
                ...
                scripts[script_name] = SkillResource(
                    path=script_path, type="script", content=None, executable=True, mode=mode, description=config.get("description"), parameters=config.get("parameters")
                )
            elif isinstance(config, str):
                # Simple path-only format: script_name: "scripts/script.py"
                full_path = skill_path / config
                if not full_path.exists():
                    logger.warning(f"Script path does not exist: {config} " f"(declared in YAML for '{script_name}')")
                    continue

                scripts[script_name] = SkillResource(path=config, type="script", content=None, executable=True, mode=self._get_default_mode(config))
            ...
            try:
                for file_path in scripts_dir.iterdir():
                    if file_path.is_file() and not file_path.name.startswith("."):
                        ...
                        scripts[script_name] = SkillResource(path=relative_path, type="script", content=None, executable=True, mode=self._get_default_mode(relative_path))
            except OSError as e:
    ...
    async def load_body(self, skill: SkillDefinition) -> str:
        ...
        try:
            ...
            raise SkillLoadError(f"Failed to load body for skill '{skill.metadata.name}': {e}") from e

    async def load_resource(self, skill: SkillDefinition, resource_type: str, resource_name: str) -> str:
        """
        ...
        if resource is None:
            raise SkillLoadError(f"Resource not found: {resource_type}/{resource_name}")
        ...
        try:
            ...
            raise SkillLoadError(f"Failed to load resource '{resource.path}': {e}") from e
