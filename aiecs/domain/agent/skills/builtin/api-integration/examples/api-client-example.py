...
class APIClient:
    ...
    def __init__(self, base_url: str, api_key: Optional[str] = None, timeout: tuple = (5, 30), max_retries: int = 3):
    ...
    def _create_session(self, max_retries: int) -> requests.Session:
        ...
        retry_strategy = Retry(total=max_retries, backoff_factor=1, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=["HEAD", "GET", "OPTIONS", "POST", "PUT", "DELETE"])

        adapter = HTTPAdapter(max_retries=retry_strategy)
        session.mount("http://", adapter)
        session.mount("https://", adapter)

        session.headers.update({"User-Agent": "APIClient/1.0", "Accept": "application/json", "Content-Type": "application/json"})
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> APIResponse:
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                return APIResponse(success=False, error=f"Rate limited. Retry after {retry_after}s", status_code=429)
            ...
            return APIResponse(success=False, error=str(e), status_code=e.response.status_code if e.response else 0)
        except ValueError:
