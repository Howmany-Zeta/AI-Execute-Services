    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if len(row) != column_count:
                return {"valid": False, "error": f"Inconsistent column count at row {row_count}"}
...
def validate_file(file_path: str, expected_format: str | None = None) -> dict:
    ...
    if expected_format and detected_format != expected_format:
        return {"valid": False, "error": f"Format mismatch: expected {expected_format}, detected {detected_format}"}
