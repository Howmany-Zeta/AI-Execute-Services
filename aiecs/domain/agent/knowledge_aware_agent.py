...
class KnowledgeAwareAgent(HybridAgent):
    ...
    async def _initialize(self) -> None:
        ...
        logger.info(
            f"KnowledgeAwareAgent {self.agent_id} initialized with enhanced capabilities"
        )
...
"""
    ...
    async def _select_tools_with_graph_awareness(
        ...
        graph_keywords = [
            "connected", "connection", "relationship", "related",
            "knows", "works", "friend", "colleague", "partner",
            "how", "why", "who", "what", "which",
            "find", "discover", "explore", "trace"
        ]
        ...
        # If graph reasoning is available and task seems graph-related, prioritize it
    ...
    async def _augment_prompt_with_knowledge(
        ...
        for query, kg_context in self._knowledge_context.items():
            ...
            if any(word in task.lower() for word in query.lower().split()):
                relevant_knowledge.append(f"- {query}: {kg_context['answer']} (confidence: {kg_context['confidence']:.2f})")
    ...
    async def execute_task(
        ...
        if self.graph_store is not None and self.enable_graph_reasoning:
            ...
            graph_keywords = ["connected", "connection", "relationship", "knows", "works at"]
    ...
    async def _retrieve_relevant_knowledge(
        ...
        session_id = context.get("session_id", f"temp_{self.agent_id}")
        ...
        try:
            ...
            if hasattr(self, '_knowledge_context') and self._knowledge_context:
                ...
                for _, kg_ctx in self._knowledge_context.items():
                    # This is simplified - in production would extract entity IDs properly
                    pass
