            ...
            try:
                ...
                if import_item:
                    if not hasattr(mod, import_item):
                        self.import_errors.append((module_name, f"{import_module}.{import_item}", AttributeError(f"{import_module} has no attribute '{import_item}'")))
                        all_ok = False
            except (ImportError, ModuleNotFoundError) as e:
                # Check if it's an internal error
                if self.is_internal_import_error(e):
                    self.import_errors.append((module_name, import_module, e))
                    all_ok = False
...
It will catch import errors before you publish, saving you from broken releases.
    ...
    parser.add_argument("--verbose", "-v", action="store_true", help="Show verbose output including successful imports")

    parser.add_argument("--module", "-m", type=str, help="Check only modules matching this filter (e.g., 'llm' to check llm.* modules)")
