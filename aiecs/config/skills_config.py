...
class SkillsConfig(BaseSettings):
    ...
    def validate_max_concurrent(cls, v: int) -> int:
        ...
        if v > 100:
            logger.warning(f"AIECS_SKILL_MAX_CONCURRENT_DISCOVERY is {v}, " "which may cause resource issues. Consider using <= 100.")
        return v
    ...
    def validate_max_timeout(cls, v: int) -> int:
        ...
        if v > 3600:
            logger.warning(f"AIECS_SKILL_MAX_TIMEOUT is {v}s (> 1 hour), " "which may cause issues with long-running scripts.")
        return v
    ...
    def validate_python_mode(cls, v: str) -> str:
        ...
        if v.lower() not in valid_modes:
            raise ValueError(f"Invalid AIECS_SKILL_DEFAULT_PYTHON_MODE: {v}. " f"Must be one of: {', '.join(valid_modes)}")
        return v.lower()
...
def validate_skills_config() -> bool:
    ...
    if not valid:
        if invalid:
            raise ValueError(f"No valid skill directories found. " f"Invalid directories: {[str(p) for p in invalid]}")
        else:
            # No directories configured at all - this is okay, just log
            logger.info("No skill directories configured. " "Skills system will start with empty registry.")
