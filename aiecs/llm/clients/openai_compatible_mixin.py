...
class OpenAICompatibleFunctionCallingMixin:
    ...
    def _convert_messages_to_openai_format(self, messages: List[LLMMessage]) -> List[Dict[str, Any]]:
        ...
        for msg in other_messages:
            ...
            if msg.tool_calls:
                sanitized = self._sanitize_tool_calls(msg.tool_calls)  # type: ignore[attr-defined]
                if sanitized:
    ...
    def _prepare_function_calling_params(
        ...
        if tools:
            params["tools"] = self._sanitize_tools(tools)  # type: ignore[attr-defined]
            if params["tools"] and tool_choice is not None:
                params["tool_choice"] = tool_choice
        elif functions:
            # Legacy format - convert to tools format for consistency
            params["tools"] = self._sanitize_tools([{"type": "function", "function": func} for func in functions])  # type: ignore[attr-defined]
            if params["tools"] and tool_choice is not None:
                params["tool_choice"] = tool_choice
