...
class LLMIntegrationManager:
    ...
    async def generate_with_context(
        ...
        **kwargs
        ...
        # Priority logic: static fallback overrides context if provided
        if fallback_provider:
            # Static configuration takes priority over context
            final_provider = fallback_provider
            final_model = fallback_model
        else:
            # Use context preferences or default to Vertex (which should be available)
            final_provider = context_provider or AIProvider.VERTEX
    ...
    def generate_with_context_sync(
        self,
        messages: Union[str, List[LLMMessage]],
        context: Union[TaskContext, Dict[str, Any]],
        fallback_provider: Optional[AIProvider] = None,
        ...
        **kwargs
        ...
        """
        Synchronous version of generate_with_context for compatibility.
        ...
        try:
            return loop.run_until_complete(
                self.generate_with_context(
                    messages, context, fallback_provider, fallback_model,
