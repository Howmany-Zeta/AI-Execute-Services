...
class MetaArchitectAgent(BaseAgent):
    ...
    async def execute_task(self, task_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # DEBUG: Log raw input data from mining service
            logger.debug(f"[META_ARCHITECT_DEBUG] Raw task_data received: {task_data}")
            ...
            # DEBUG: Log extracted mining context
            logger.debug(f"[META_ARCHITECT_DEBUG] Enhanced mining context extracted: {mining_context}")
            logger.debug(f"[META_ARCHITECT_DEBUG] Mining context type: {type(mining_context)}")
            logger.debug(f"[META_ARCHITECT_DEBUG] Has enhanced context: {mining_context.get('has_enhanced_context', False)}")
    ...
    async def _execute_decomposition_task(self, problem_description: str, context: Dict[str, Any],
        ...
        logger.debug(f"[META_ARCHITECT_DEBUG] _execute_decomposition_task - Complete decomposition input being sent to LLM:")
        logger.debug(f"[META_ARCHITECT_DEBUG] Input: {decomposition_input.get('input', 'N/A')}")
        logger.debug(f"[META_ARCHITECT_DEBUG] Task Description: {decomposition_input.get('task_description', 'N/A')}")
        logger.debug(f"[META_ARCHITECT_DEBUG] Expected Output: {decomposition_input.get('expected_output', 'N/A')}")
        logger.debug(f"[META_ARCHITECT_DEBUG] Input Data: {decomposition_input.get('input_data', 'N/A')}")
        logger.debug(f"[META_ARCHITECT_DEBUG] Full decomposition_input: {decomposition_input}")
        ...
        logger.debug(f"[META_ARCHITECT_DEBUG] _execute_decomposition_task - Raw LLM result: {result}")
        logger.debug(f"[META_ARCHITECT_DEBUG] _execute_decomposition_task - Result type: {type(result)}")

        # Extract the actual output from LangChain result
        actual_output = result.get('output', str(result))
        logger.debug(f"[META_ARCHITECT_DEBUG] _execute_decomposition_task - Extracted actual_output: {actual_output}")
        logger.debug(f"[META_ARCHITECT_DEBUG] _execute_decomposition_task - Actual output type: {type(actual_output)}")
        ...
        if isinstance(actual_output, str):
            try:
                ...
                parsed_result = self._create_fallback_decomposition_result(problem_description, actual_output, mining_context)
        else:
    ...
    async def _execute_enhanced_blueprint_construction_task(self, problem_description: str, domain: str,
        ...
        if isinstance(actual_output, str):
            try:
                ...
                parsed_result = self._create_enhanced_fallback_blueprint_result(
                    problem_description, domain, complexity, mining_context, actual_output
                )
        else:
            parsed_result = actual_output

        # Validate context usage
        validation_result = self._validate_context_usage(parsed_result, problem_description, mining_context)
        if not validation_result["is_valid"]:
            logger.warning(f"Context validation failed: {validation_result['issues']}")
            # Add validation feedback to result
            parsed_result["validation_feedback"] = validation_result
    ...
    async def _recursive_construct_plan(self, problem_description: str, domain: str, complexity: str,
        ...
        # DEBUG: Log enhanced mining context before decomposition
        logger.debug(f"[META_ARCHITECT_DEBUG] _recursive_construct_plan - Enhanced mining context at depth {current_depth}: {enhanced_mining_context}")
        logger.debug(f"[META_ARCHITECT_DEBUG] _recursive_construct_plan - Problem description: {problem_description}")
        logger.debug(f"[META_ARCHITECT_DEBUG] _recursive_construct_plan - Domain: {domain}, Complexity: {complexity}")
    ...
    def _extract_mining_context(self, requirements: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract enhanced mining context from requirements (legacy compatibility method).
        This method is now mainly for backward compatibility when mining_context is not directly provided.

        Args:
            requirements: Requirements dictionary (legacy structure)
        ...
        # First check if mining_context is nested under requirements
        nested_mining_context = requirements.get("mining_context", {})

        # DEBUG: Log nested mining context
        logger.debug(f"[META_ARCHITECT_DEBUG] _extract_mining_context - Nested mining context: {nested_mining_context}")

        # Extract from both top-level and nested structure for compatibility
        mining_context = {
            "demand_state": requirements.get("demand_state") or nested_mining_context.get("demand_state"),
            "smart_analysis": requirements.get("smart_analysis", {}) or nested_mining_context.get("smart_analysis", {}),

            # Handle new field names from updated mining service
            "intent_parsing_reasoning": requirements.get("intent_parsing_reasoning", "") or nested_mining_context.get("intent_parsing_reasoning", ""),
            "intent_parsing_output": requirements.get("intent_parsing_output", "") or nested_mining_context.get("intent_parsing_output", ""),
            "complexity": requirements.get("complexity", "medium") or nested_mining_context.get("complexity", "medium"),

            # Legacy fields for backward compatibility
            "entities_keywords": requirements.get("entities_keywords", {}) or nested_mining_context.get("entities_keywords", {}),
            "analysis_focus": requirements.get("analysis_focus", []) or nested_mining_context.get("analysis_focus", []),
            "framework_hints": requirements.get("framework_hints", []) or nested_mining_context.get("framework_hints", []),
            "clarification_history": requirements.get("clarification_history", []) or nested_mining_context.get("clarification_history", []),
            "has_enhanced_context": nested_mining_context.get("has_enhanced_context", True) if nested_mining_context else bool(requirements.get("intent_categories") or requirements.get("smart_analysis"))
        }

        # DEBUG: Log extracted individual components
        logger.debug(f"[META_ARCHITECT_DEBUG] _extract_mining_context - Extracted components:")
        logger.debug(f"  - demand_state: {mining_context.get('demand_state')}")
        logger.debug(f"  - smart_analysis: {mining_context.get('smart_analysis')}")
        logger.debug(f"  - intent_parsing_reasoning: {mining_context.get('intent_parsing_reasoning')}")
        logger.debug(f"  - intent_parsing_output: {mining_context.get('intent_parsing_output')}")
        logger.debug(f"  - complexity: {mining_context.get('complexity')}")
        logger.debug(f"  - entities_keywords: {mining_context.get('entities_keywords')}")
        logger.debug(f"  - analysis_focus: {mining_context.get('analysis_focus')}")
        logger.debug(f"  - framework_hints: {mining_context.get('framework_hints')}")
        logger.debug(f"  - clarification_history: {mining_context.get('clarification_history')}")
        logger.debug(f"  - has_enhanced_context: {mining_context.get('has_enhanced_context')}")
