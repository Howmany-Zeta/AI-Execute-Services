...
class DSLProcessor:
    """
    专门处理 DSL (Domain Specific Language) 解析和执行
    """
    ...
    def evaluate_condition(self, condition: str, intent_categories: List[str],
        ...
        """
        try:
            # Intent 条件: intent.includes('category')
            match = re.match(r"intent\.includes\('([^']+)'\)", condition)
            if match:
                category = match.group(1)
                return category in intent_categories

            # Context 条件: context.field == value
            match = re.match(r"context\.(\w+)\s*(==|!=|>|<|>=|<=)\s*(.+)", condition)
            if match and context:
                field, operator, value = match.groups()
                return self._evaluate_comparison(context.get(field), operator, self._parse_value(value))

            # Input 条件: input.field == value
            match = re.match(r"input\.(\w+)\s*(==|!=|>|<|>=|<=)\s*(.+)", condition)
            if match and input_data:
                field, operator, value = match.groups()
                return self._evaluate_comparison(input_data.get(field), operator, self._parse_value(value))

            # Result 条件: result[0].field == value
            match = re.match(r"result\[(\d+)\]\.(\w+)\s*(==|!=|>|<|>=|<=)\s*(.+)", condition)
        ...
        try:
            ...
            if condition_result:
                ...
                return TaskStepResult(
                    ...
                    status=TaskStatus.COMPLETED if all(r.status == TaskStatus.COMPLETED for r in step_results) else TaskStatus.FAILED
                )
            else:
                if span:
            ...
            return TaskStepResult(
                ...
                status=TaskStatus.FAILED,
                error_code=ErrorCode.DSL_EVALUATION_ERROR,
                error_message=str(e)
            )
    ...
    async def _handle_parallel_step(self, step: Dict, input_data: Dict, context: Dict,
        ...
        return TaskStepResult(
            ...
            status=TaskStatus.COMPLETED if all(r.status == TaskStatus.COMPLETED for r in batch_results) else TaskStatus.FAILED
        )
    ...
    def get_supported_features(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_value_types": ["string", "number", "boolean"]
        }
