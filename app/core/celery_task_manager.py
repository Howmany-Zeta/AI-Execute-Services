...
from app.tasks.worker import execute_task, execute_heavy_task
        ...
        try:
            if queue == 'fast_tasks':
                result = execute_task.apply_async(
                    args=(task_name, user_id, task_id, step, mode, service, input_data, context),
                    queue=queue
                )
            else:
                result = execute_heavy_task.apply_async(
                    args=(task_name, user_id, task_id, step, mode, service, input_data, context),
                    queue=queue
                )
